...
13. `/tasks` 是否仍把 workspace/thread 动态值限制在 `plain_text`，并在 50 任务上限后使用 overflow section；“工作区与会话”父页入口是否仍走 `enter_terminal + ReplaceCurrentCard`，而不是 append 第二张卡或建立新的 owner
